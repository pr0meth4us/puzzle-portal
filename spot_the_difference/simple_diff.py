...
simple_diff.py
--------------
Automatic spot-the-difference detector.
Robust alignment is the entire focus.
"""
...
HUE_DILATE_KSIZE     = 9     # smaller kernel prevents blob fusion
HUE_SCORE_WEIGHT     = 0.5
MAX_BLOB_RADIUS_FRAC = 0.25
MERGE_RADIUS         = 55    # tighter merging
LOW_DELTA_FRAC       = 0.40
CIRCLE_PAD           = 6     # tighter circles
...
def auto_slice(img: np.ndarray):
    ...
    # 1. Search Horizontal Cuts in [0.45 * h, 0.55 * h]
    ...
    # 2. Search Vertical Cuts in [0.45 * w, 0.55 * w]
...
def detect(img_a: np.ndarray,
    ...
    for cnt, delta, cx, cy, r in surviving:
        ...
        merged = False
        for grp in groups:
            if GrpDist := (((cx - grp[2]) ** 2 + (cy - grp[3]) ** 2) ** 0.5) < MERGE_RADIUS:
    ...
    for grp in groups:
        ...
        if len(centres) == 1:
            cx, cy = centres[0]
            r = max(int(max_sub_r) + CIRCLE_PAD, 18)
        else:
            (cx, cy), span = cv2.minEnclosingCircle(centres.reshape(-1, 1, 2))
            r = max(int(span + max_sub_r) + CIRCLE_PAD, 18)
        if r > max_allowed_r:
            r = max_allowed_r
...
def parse_args():
    ...
    p.add_argument("--delta-floor", type=float, default=7.0)
    p.add_argument("--mode",
...
def main():
    ...
    if args.output is None:
        args.output = str(Path(__file__).parent / "results" / "differences_circled_new.jpg")
