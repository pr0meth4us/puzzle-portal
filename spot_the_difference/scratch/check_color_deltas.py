...
diff_L = np.abs(la[:, :, 0] - lb[:, :, 0])
diff_ab = np.sqrt(np.sum((la[:, :, 1:] - lb[:, :, 1:]) ** 2, axis=2))
...
for cnt in contours:
    ...
    mean_rgb_diff = cv2.mean(cdiff_rgb, mask=m)[0]
    mean_L_diff = cv2.mean(diff_L, mask=m)[0]
    mean_ab_diff = cv2.mean(diff_ab, mask=m)[0]
    ...
    candidates.append((cx, cy, r, mean_rgb_diff, mean_L_diff, mean_ab_diff, gt_label))

candidates.sort(key=lambda x: x[6])
print(f"{'Label':<8} | {'Center':<14} | {'rgb_diff':<8} | {'L_diff':<8} | {'ab_diff':<8}")
print("-" * 55)
for cx, cy, r, rgb, L, ab, lbl in candidates:
    print(f"{lbl:<8} | ({cx:4.1f},{cy:4.1f}) | {rgb:8.2f} | {L:8.2f} | {ab:8.2f}")
