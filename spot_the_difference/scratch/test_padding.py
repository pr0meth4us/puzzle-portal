...
# Let's run detect with margin padding = 0 and edge_mask_ksize = 0
def test_padding(pad_val):
    print(f"\n--- Testing with margin padding = {pad_val} ---")
    ...
    # Custom margin masking with pad_val
    ...
    for idx, (cnt, delta, cx, cy, r) in enumerate(candidates):
        # Only print those with delta >= 25.0
        if delta >= 25.0:
