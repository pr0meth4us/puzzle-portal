...
# Find candidates sorted by distance to (45, 421)
print("Candidates sorted by distance to (45, 421):")
cands_with_dist = []
for idx, (cnt, delta, cx, cy, r) in enumerate(candidates):
    dist = np.sqrt((cx - 45)**2 + (cy - 421)**2)
    cands_with_dist.append((dist, idx, cx, cy, r, delta))

cands_with_dist.sort(key=lambda x: x[0])
for rank, (dist, idx, cx, cy, r, delta) in enumerate(cands_with_dist[:10]):
    print(f"Rank {rank+1:2d} | dist={dist:6.1f} | Center=({cx:4d}, {cy:4d}), r={r:3d}, delta={delta:.2f}")
