...
# Let's write a custom detect that uses the new margin warping
def custom_detect():
    ...
    # 1. Detect margins on A
    # We will copy the margin detection code from _mask_margins but get the raw bounds
    ...
    # 2. If H is present, also mask margins in B's space and warp it
    if H_align is not None:
        bmask_b = np.ones((h, w), dtype=np.uint8) * 255
        if left_spike > 0:
            bmask_b[:, :left_spike + 8] = 0
        if right_spike < w:
            bmask_b[:, right_spike - 8:] = 0
        if top_spike > 0:
            bmask_b[:top_spike + 8, :] = 0
        if bottom_spike < h:
            bmask_b[bottom_spike - 8:, :] = 0
        ...
        # Also warp B's canvas mask
        mask_b = np.ones((h, w), dtype=np.uint8) * 255
        warped_canvas_b = cv2.warpPerspective(mask_b, H_align, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (11, 11))
        warped_canvas_b = cv2.erode(warped_canvas_b, kernel)
        
        # Combine everything
        bmask = cv2.bitwise_and(bmask, warped_bmask_b)
        bmask = cv2.bitwise_and(bmask, warped_canvas_b)
    ...
    for idx, (cnt, delta, cx, cy, r) in enumerate(candidates):
        print(f"Cand {idx+1}: Center=({cx}, {cy}), r={r}, delta={delta:.2f}")
