...
def test_margin_pad(pad_val, df_val):
    ...
    print(f"Pad_val={pad_val}, df={df_val:.1f} -> Groups={len(groups)}, Matched={len(matched_gt)}/12, FP={fps}, Missed={[i+1 for i in range(12) if i not in matched_gt]}")

for pv in [0, 2, 4, 6, 8]:
    for df in [10.0, 12.0, 15.0]:
        test_margin_pad(pv, df)
