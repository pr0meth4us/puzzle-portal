...
thresh_base = cv2.bitwise_or(thresh_ssim, thresh_lab)
thresh_base = cv2.bitwise_or(thresh_base, thresh_hue)

bmask_ocr = np.ones((h, w), dtype=np.uint8) * 255
try:
    import pytesseract
    ...
    for img in (img_a, img_b_aligned):
        ...
        for i in range(n_boxes):
            ...
            if text:
                ...
                w_box = data['width'][i]
                h_box = data['height'][i]
                if w_box > bmask_ocr.shape[1] * 0.35 or h_box > bmask_ocr.shape[0] * 0.25:
                    continue
                if x > w_panel - 180:
                    continue
                pad = 12
                x1 = max(0, x - pad)
                y1 = max(0, y - pad)
                x2 = min(bmask_ocr.shape[1], x + w_box + pad)
                y2 = min(bmask_ocr.shape[0], y + h_box + pad)
                bmask_ocr[y1:y2, x1:x2] = 0
...
if H_align is not None:
    ...
    bmask_ocr = cv2.bitwise_and(bmask_ocr, warped_mask_b)

def test_x_mask(left_mask_x, df_val):
    bmask = bmask_ocr.copy()
    bmask[:, :left_mask_x] = 0
    bmask[:, 1152-30:] = 0  # right margin
    bmask[:30, :] = 0       # top margin
    bmask[1006-30:, :] = 0  # bottom margin
    ...
    thresh = cv2.bitwise_and(thresh_base, bmask)
    ...
    print(f"left_mask_x={left_mask_x}, df={df_val:.1f} -> Groups={len(groups)}, Matched={len(matched_gt)}/12, FP={fps}, Missed={[i+1 for i in range(12) if i not in matched_gt]}")

for lx in [36, 40, 44, 48, 52]:
    for df in [10.0, 12.0, 15.0]:
        test_x_mask(lx, df)
