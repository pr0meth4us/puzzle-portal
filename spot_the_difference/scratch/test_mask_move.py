...
def improved_mask_margins(img: np.ndarray, bmask: np.ndarray):
    try:
        h, w = img.shape[:2]
        ...
        # Mask with border padding (+8 pixels) to cleanly clip SIFT warping noise
        if left_spike > 0:
        ...
        for img in (img_a, img_b):
            ...
            for i in range(n_boxes):
                ...
                if text:
                    ...
                    # Check for aggressive corner masking (watermark/metadata)
                    if x < 250 and y > bmask.shape[0] - 120:
                        x1 = 0
                        y1 = max(0, bmask.shape[0] - 120)
                        x2 = 250
                        y2 = bmask.shape[0]
                        bmask[y1:y2, x1:x2] = 0
                        print(f"[INFO] OCR text masking: Aggressive bottom-left corner mask applied due to '{text}'")
                    elif x > bmask.shape[1] - 250 and y > bmask.shape[0] - 120:
                        x1 = bmask.shape[1] - 250
                        y1 = max(0, bmask.shape[0] - 120)
                        x2 = bmask.shape[1]
                        y2 = bmask.shape[0]
                        bmask[y1:y2, x1:x2] = 0
                        print(f"[INFO] OCR text masking: Aggressive bottom-right corner mask applied due to '{text}'")
                    else:
                        pad = 4
                        x1 = max(0, x - pad)
                        y1 = max(0, y - pad)
...
def detect_with_moved_mask(img_a: np.ndarray,
    ...
    # ── CREATE AND POPULATE bmask BEFORE MORPHOLOGY ─────────────────────────
    ...
    improved_mask_margins(img_a, bmask)
    improved_mask_ocr_text(img_a, img_b, bmask)
    
    # Apply bmask before morphology
    ...
    # Apply bmask again after morphology
    thresh = cv2.bitwise_and(thresh, bmask)
...
def main():
    for p_num in ("07", "08"):
        print(f"\n--- TESTING PUZZLE {p_num} WITH IMPROVED MASKING ---")
        ...
        circles, count = detect_with_moved_mask(
            img_a, img_b_aligned,
            min_area=50,
            delta_floor=7.0,
