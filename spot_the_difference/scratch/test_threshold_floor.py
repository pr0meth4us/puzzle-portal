...
print(f"[DEBUG] Default Otsu threshold: SSIM={otsu_ssim:.1f}, Lab={otsu_lab:.1f}")

# Pre-mask margins
bmask_base = np.ones((h, w), dtype=np.uint8) * 255
std._mask_margins(img_a, bmask_base)

# Pre-mask OCR text (excluding right edge figure label)
try:
    import pytesseract
    ...
    for img in (img_a, img_b_aligned):
        ...
        for i in range(n_boxes):
            ...
            if text:
                ...
                w_box = data['width'][i]
                h_box = data['height'][i]
                if w_box > bmask_base.shape[1] * 0.35 or h_box > bmask_base.shape[0] * 0.25:
                    continue
                if x > w_panel - 180:
                    continue
                pad = 12
                x1 = max(0, x - pad)
                y1 = max(0, y - pad)
                x2 = min(bmask_base.shape[1], x + w_box + pad)
                y2 = min(bmask_base.shape[0], y + h_box + pad)
                bmask_base[y1:y2, x1:x2] = 0
...
if H_align is not None:
    ...
    bmask_base = cv2.bitwise_and(bmask_base, warped_mask_b)

def run_with_threshold_floors(ssim_floor, lab_floor):
    use_ssim_thresh = max(otsu_ssim, ssim_floor)
    use_lab_thresh = max(otsu_lab, lab_floor)

    _, thresh_ssim = cv2.threshold(inv,      use_ssim_thresh, 255, cv2.THRESH_BINARY)
    _, thresh_lab  = cv2.threshold(lab_diff, use_lab_thresh,  255, cv2.THRESH_BINARY)

    thresh = cv2.bitwise_or(thresh_ssim, thresh_lab)
    thresh = cv2.bitwise_or(thresh,      thresh_hue)

    bmask_run = bmask_base.copy()
    if vy0 > 16:
        bmask_run[:vy0, :] = 0
    if vy1 < h - 8:
        bmask_run[vy1:,  :] = 0
    ...
    for df in [8.0, 10.0, 12.0, 14.0, 15.0, 16.0, 18.0]:
        ...
        if len(matched_gt) == 12 and fps == 0:
            print(f"SUCCESS: ssim_floor={ssim_floor}, lab_floor={lab_floor}, df={df:.1f} -> Groups={len(groups)}, Matched={len(matched_gt)}/12, FP={fps}")
            return True
            
    # Print status for df=12.0
    surviving = [c for c in candidates if c[1] >= 12.0]
    groups = []
    for cnt, delta, cx, cy, r in surviving:
        cx, cy = float(cx), float(cy)
        merged = False
    ...
    for grp in groups:
        ...
        if closest_dist < 60:
            ...
            fps += 1
    print(f"ssim_floor={ssim_floor}, lab_floor={lab_floor} -> df=12.0: Matched={len(matched_gt)}/12, FP={fps}, Missed={[i+1 for i in range(12) if i not in matched_gt]}")
    return False

# Sweep floors
for sf in [25, 40, 50, 60, 70, 80, 90]:
    for lf in [38, 45, 50, 60, 70, 80, 90]:
        run_with_threshold_floors(sf, lf)
