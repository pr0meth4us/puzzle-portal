...
# Custom OCR masking (skips figure label)
try:
    import pytesseract
...
print(f"\nDetections and GT Matches (delta_floor=12.0):")
matched_gt = set()
for idx, grp in enumerate(groups):
    ...
    if closest_dist < 60:
        ...
        print(f"  Group {idx+1}: Center=({cx:.1f}, {cy:.1f}), Delta={grp[1]:.2f} -> MATCHED GT {closest_gt_idx+1} at ({truth_actual[closest_gt_idx][0]:.1f}, {truth_actual[closest_gt_idx][1]:.1f}) dist={closest_dist:.1f}")
    else:
        print(f"  Group {idx+1}: Center=({cx:.1f}, {cy:.1f}), Delta={grp[1]:.2f} -> FALSE POSITIVE (closest GT {closest_gt_idx+1} dist={closest_dist:.1f})")

print(f"\nMissed GTs:")
for i, gt in enumerate(truth_actual):
    if i not in matched_gt:
        print(f"  Missed GT {i+1} at {gt}")
