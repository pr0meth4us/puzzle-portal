...
# Corrected ground truth dots in img_a space
truth_actual = [
    ...
    (1036.0, 430.0)   # 12. figure label at right edge (corrected centroid)
]
...
def test_ksize(close_k, open_k, df_val):
    ...
    print(f"Close_k={close_k}, Open_k={open_k}, df={df_val:.1f} -> Groups={len(groups)}, Matched={len(matched_gt)}/12, FP={fps}, Missed={[i+1 for i in range(12) if i not in matched_gt]}")
