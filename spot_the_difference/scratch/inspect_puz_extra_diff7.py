...
print(f"Difference 7 ROI details:")
print(f"  Center: ({cx}, {cy}), r: {r}")
print(f"  max_val: {max_val}, thresh_val: {thresh_val}")
print(f"  Total contours found: {len(cnts)}")
for i, c in enumerate(cnts):
    print(f"    Contour {i+1}: area={cv2.contourArea(c):.1f}")
