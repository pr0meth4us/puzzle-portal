...
SESSION_DIR = SCRIPT_DIR / "fb_session_bot"
STATE_FILE = SCRIPT_DIR / "fb_state_bot.json"

TARGET_URL = "https://www.messenger.com/t/1276957488995520"
...
async def get_gemini_answer_from_screenshot(image_bytes):
    ...
    system_instruction = (
        ...
        "10. If it is a genuine math puzzle, logic puzzle, or text riddle that you can solve right now, output ONLY the final concise answer. DO NOT be conversational. DO NOT explain your reasoning. JUST answer the riddle.\n"
        ...
        "12. **SENDER RESTRICTION**: You must ONLY solve NEW riddles if they are posted by 'Ron' or 'Ron Oum'. If another person posts a brand new riddle, output IGNORE. HOWEVER, if another person posts an ANSWER to Ron's visual puzzle, you MUST steal their answer as described in Rule 9!\n"
...
async def run_bot(headed=True):
    print("\n=== Playwright: Facebook Messenger Bot ===")
    
    # IGNORING OLD STATE: Force a fresh login
    ...
    async with async_playwright() as p:
        ...
        try:
            print(f"Navigating to Messenger chat: {TARGET_URL}...")
            ...
            if "login" in page.url or await page.locator("input[name='email']").is_visible():
                ...
                try:
                    ...
                    # Re-navigate to target URL just in case login redirected us to the homepage
                    await page.goto(TARGET_URL, wait_until="domcontentloaded", timeout=30000)
                    await page.wait_for_timeout(_human_delay(3000, 5000))
            ...
            while True:
                try:
                    # Only look at the bottom 5 rows
                    ...
                    for row in bottom_rows:
                        is_processed = await row.evaluate('node => node.hasAttribute("data-bot-processed")')
                        if not is_processed:
                            # Mark as processed immediately
                            await row.evaluate('node => node.setAttribute("data-bot-processed", "true")')
